$(document).ready(function () {
    function loadClients(page = 1) {
        ...
        $.ajax({
            url: "/uzart/www/api/clients_api.php",
            type: "GET",
            data: formData,
            dataType: "json",
			...
			let blurredKey = "•".repeat(client.api_key.length); // API Key 블러 처리
                        tableBody.append(`
                            <tr>
                                <td>${client.client}</td>
                                <td>${client.server_name}</td>
                                <td>
				    <span class="api-key blurred" data-key="${client.api_key}">${blurredKey}</span>
				</td>
                                <td>
                                    <button class="download-btn" data-client="${client.client}" data-server="${client.server_name}">📥</button>
                                </td>
                            </tr>
                        `);
                    });
...
//    });
    ...
    // 조회 버튼 클릭 시
    $("#clientFilterForm").submit(function (event) {
